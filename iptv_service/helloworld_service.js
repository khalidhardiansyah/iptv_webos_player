...
function StalkerClient(baseUrl, mac) {
	...
	this.mac = mac;
	this.token = null;
	// IMPORTANT: Use Europe/Paris timezone like working app (will be URL-encoded automatically)
	this.cookies = ['mac=' + encodeURIComponent(mac), 'stb_lang=en', 'timezone=Europe/Paris'];
	console.log('[StalkerClient] Created client for MAC:', mac);
...
StalkerClient.prototype.makeRequest = function(action, params, method) {
    ...
    // List of potential Stalker API endpoints to try
    var endpoints = [
        'server/load.php',
        'portal.php',
    ...
    // Use current working endpoint if known, otherwise start at 0
    var startIdx = (self.currentEndpointIndex !== undefined) ? self.currentEndpointIndex : 0;

    function tryEndpoint(idx) {
        if (idx >= endpoints.length) {
            return Promise.reject(new Error('All Stalker endpoints failed after retries'));
        }

        var endpoint = endpoints[idx];
        console.log('[StalkerClient] Attempting request on:', endpoint);

        return new Promise(function(resolve, reject) {
            // Internal function to handle request with redirect support
            function doRequest(currentUrl, redirectCount) {
                ...
                try {
                    ...
                    console.log('[StalkerClient] REQUEST (' + redirectCount + ') - Action:', action, 'URL:', fullUrl.substring(0, 100));
                    
                    var isHttps = fullUrl.startsWith('https://');
                    var protocol = isHttps ? https : http;
                    var urlWithoutProtocol = fullUrl.replace(/^https?:\/\//, '');
                    ...
                    // Standardized Referer Logic (WAF Bypass)
                    ...
                    if (self.token) requestOptions.headers['Authorization'] = 'Bearer ' + self.token;
                    
                    var req = protocol.request(requestOptions, function(res) {
                        var data = '';
                        console.log('[StalkerClient] Status:', res.statusCode);
                            ...
                            if (redirectUrl) {
                                if (res.headers['set-cookie']) {
                                    ...
                                    setCookies.forEach(function(cookieStr) {
                                        ...
                                        var cookieName = cookieValue.split('=')[0].trim();
                                        self.cookies = self.cookies.filter(function(c) { return !c.startsWith(cookieName + '='); });
                        ...
                        res.on('end', function() {
                            ...
                            try {
                                if (!data || data.trim().length === 0) throw new Error('Empty response');
                                resolve(JSON.parse(data));
                            } catch (e) {
                                reject(new Error('JSON Parse Error: ' + e.message + ' Data: ' + data.substring(0, 50)));
                            }
                        });
                    });
            ...
            // FAILURE: Try next endpoint if we haven't exhausted them or if it's a specific "WAF" or "Endpoint" error
            console.warn('[StalkerClient] Action ' + action + ' failed on ' + endpoint + ':', err.message);
            
            var isRetryable = err.message.includes('404') || err.message.includes('444') || err.message.includes('403') || err.message.includes('405') || action === 'handshake';
...
StalkerClient.prototype.handshake = function() {
	var self = this;
	console.log('[StalkerClient] Handshake...');
	...
	return this.makeRequest('stb', {
		...
		if (data && data.js && data.js.token) {
			...
			console.log('[StalkerClient] Token:', self.token.substring(0, 20) + '...');
		}
		return data;
	});
};
