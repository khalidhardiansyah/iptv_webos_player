...
class App {
  constructor() {
    ...
    this.screenHistory = [];
    this.stalkerClient = new StalkerClient();
  ...
  renderCategories(genres) {
    ...
    genres.forEach((genre, index) => {
      ...
      if (index === 0 && genres.length === this.allGenres.length && !this.selectedCategory) {
          setTimeout(() => this.selectCategory(genre, item), 300);
      }
    });
  ...
  async loadVodItems(categoryId, categoryTitle) {
    ...
    container.innerHTML = '<div class="loading"><div class="spinner"></div><div class="loading-text">Loading items...</div></div>';
  ...
  setupVodScroll() {
      ...
      container.addEventListener('scroll', () => {
          if (!container.classList.contains('vod-grid')) return;
  ...
  async loadChannels(genreId, genreTitle) {
    ...
    try {
      const channels = await this.stalkerClient.getChannels(genreId);
  ...
  async searchChannels(query) {
    if (this.pendingSearchRequest) {
      this.pendingSearchRequest.aborted = true;
      this.pendingSearchRequest = null;
    }
    const requestTracker = { aborted: false };
    this.pendingSearchRequest = requestTracker;
    document.getElementById("channels-title").textContent = "Search: " + query;
    const container = document.getElementById("channels-container");
    container.innerHTML = "<div class=\"loading\"><div class=\"spinner\"></div></div>";
    
    try {
      const channels = await this.stalkerClient.search(query);
      if (requestTracker.aborted) return;
      if (!channels || channels.length === 0) {
      ...
      if (!requestTracker.aborted) container.innerHTML = "<div class=\"error-message\">Search failed</div>";
    } finally {
      if (this.pendingSearchRequest === requestTracker) this.pendingSearchRequest = null;
    }
  }
  ...
  async playChannel(channel, channelIndex = -1, force = false) {
    ...
    // Don't close overlay if it's a retry (force=true)
    if (!force && this.isOverlayMode) this.closeOverlay();
    ...
    try {
      ...
      if (mpegts.isSupported()) {
        ...
        this.mpegtsPlayer = mpegts.createPlayer({
            ...
            cors: true // Enable CORS for proxy
        }, {
            // OPTIMIZED CONFIG FROM REFERENCE REPO
            enableWorker: true,
            lazyLoadMaxDuration: 3 * 60,
            seekType: 'range',
            liveBufferLatencyChasing: false, // Disable aggressive latency chasing
            liveBufferLatencyMaxLatency: 5, // Increased from default
            liveBufferLatencyMinRemain: 1, // Increased from default
            stashInitialSize: 1024, // 1MB for better buffering
        ...
        this.mpegtsPlayer.on(mpegts.Events.ERROR, (type, details) => {
          ...
          // Don't retry on Early-EOF, let it reconnect
          if (details && typeof details === 'string' && details.includes('Early-EOF')) {
              console.log('[mpegts] Early-EOF detected, will retry...');
              // mpegts.js will auto-retry, just log it
              return;
          }
        ...
        // IMPLEMENT STALL DETECTION FROM REFERENCE REPO
        if (this.stallInterval) clearInterval(this.stallInterval);
        
        let lastCurrentTime = 0;
        let stallCount = 0;
        const maxStallCount = 5;
        
        this.stallInterval = setInterval(() => {
            if (!video) return;
            
            // Check for unexpected 'ended' state for live streams
            if (video.ended && !video.paused) {
                console.warn('[StallDetector] Live stream ended unexpectedly, recovering...');
                this.handlePlayerError('STALL', 'STREAM_ENDED_UNEXPECTED', currentSessionId);
                return;
            }
            ...
            if (!video.paused && video.readyState >= 2) {
                if (video.currentTime === lastCurrentTime && video.currentTime > 0) {
                    stallCount++;
                    if (stallCount % 2 === 0) console.log(`[StallDetector] Stalled for ${stallCount}s`);

                    if (stallCount >= maxStallCount) {
                        console.warn('[StallDetector] Max stall count reached, recovering...');
                        this.handlePlayerError('STALL', 'PLAYBACK_STALLED', currentSessionId);
                        stallCount = 0; 
                    }
                } else {
                    if (stallCount > 0) console.log('[StallDetector] Playback resumed');
                    stallCount = 0;
  ...
  handlePlayerError(type, details, sessionId) {
    ...
    // ADDED: Special handling for Early-EOF (SOLUSI 4) and STREAM_ENDED
    if (typeof details === 'string' && (details.includes('Early-EOF') || details.includes('STREAM_ENDED'))) {
        console.log('[Error] Early-EOF/StreamEnded detected, quick retry...');
        
        if (this.retryCount < this.maxRetries) {
            this.retryCount++;
            
            // Shorter delay for Early-EOF
            setTimeout(() => {
            ...
            }, 1000);  // 1 second retry for Early-EOF
            return;
        }
    }
  ...
  async playVodItem(item, itemIndex = -1) {
    ...
    try {
      ...
      if (typeof streamUrlData === 'object' && streamUrlData.url) {
          ...
          meta = streamUrlData.meta || {};
      } else {
          streamUrl = streamUrlData;
      }

      if (!streamUrl) throw new Error("No VOD URL");
      ...
      let externalSubs = [];
      if (item.subtitles && Array.isArray(item.subtitles)) externalSubs = item.subtitles;
      if (meta.subtitles && Array.isArray(meta.subtitles)) externalSubs = meta.subtitles;
      console.log("External Subtitles Found:", externalSubs);
      

      
      let proxiedStreamUrl = streamUrl;
      console.log("[playVodItem] Client useLuna:", this.stalkerClient.useLuna);
      
      // EXPLICITLY DESTROY PLAYERS
      if (this.mpegtsPlayer) {
        this.mpegtsPlayer.destroy();
        this.mpegtsPlayer = null;
        console.log("[playVodItem] Destroyed mpegtsPlayer for VOD.");
      }
      if (this.hlsPlayer) {
        this.hlsPlayer.destroy();
        this.hlsPlayer = null;
        console.log("[playVodItem] Destroyed hlsPlayer for VOD.");
      }
      
      if (this.stalkerClient.useLuna) {
         playerLoadingText.textContent = "Setting up proxy...";
         try {
             const pUrl = await this.stalkerClient.lunaClient.streamProxy(streamUrl, this.stalkerClient.portalUrl, this.stalkerClient.mac);
             if (pUrl && pUrl !== streamUrl) {
                 proxiedStreamUrl = pUrl;
                 console.log("[playVodItem] Proxy successful:", proxiedStreamUrl);
             } else {
                 console.warn("[playVodItem] Proxy returned original URL or failed.");
             }
         } catch(e) {
             console.error("[playVodItem] Proxy call error:", e);
         }
      }
      ...
      const onLoadedMetadata = () => {
          ...
          // We check now, but they might appear later.
          if (video.textTracks && video.textTracks.length > 0) {
              console.log(`[playVodItem] Found ${video.textTracks.length} text tracks.`);
          }
          
          this.updateTrackButtons();
          setTimeout(() => this.updateTrackButtons(), 1000);
      };
