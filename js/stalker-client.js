...
class StalkerClient {
  ...
  async getChannels(genreId) {
    try {
      const data = await this.callService('channels', { genreId: genreId || '*' });
  ...
  async createLink(channelId, cmd) {
    try {
      ...
      if (streamUrl.endsWith('&play_to') || streamUrl.endsWith('&')) {
          ...
          console.log('Cleaned incomplete URL parameters:', streamUrl.substring(0, 100));
      }
  ...
  async createVodLink(cmd) {
    try {
      const response = await this.callService('vod_link', { cmd: cmd });
      ...
      console.log("VOD Stream URL:", streamUrl ? streamUrl.substring(0, 100) : 'null');
      console.log("VOD Metadata:", meta);
